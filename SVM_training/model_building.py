...
def main(arg = sys.argv):
	if len(arg) != 3:
		...
		with open(arg[2], 'r') as inf:
			...
			line = inf.readline()
			while line != '':
				featureListSelected.append((float(line.strip().split('\t')[1]), int(line.strip().split('\t')[2]), line.strip().split('\t')[0]))
				line = inf.readline()
		time1 = time.time()
		...
		gs = GridSearchCV(estimator=pipe_svc, param_grid=param_grid, scoring=mcc_scorer, cv=10, n_jobs=32)
		gs = gs.fit(x_train_std_filter, data_y)
		clf = gs.best_estimator_
		clf.fit(x_train_std_filter, data_y)
		time2 = time.time()
		print('%d\t%.3f\t%.1f'%(len(featureListSelected), gs.best_score_, time2-time1))
		"""
		...
		"""
		joblib.dump([sc, clf, featureListSelected], "human_svc_model_060320.pkl", compress=1)
